...
class Heuristic_Walk(object):
    ...
    def sample_next_edge(self, filtered_edges, cur_ts):
        """
        Define next edge distribution based on heuristic scores.
        """
        heuristic_scores = self.compute_heuristic_scores(filtered_edges)
        probabilities = heuristic_scores / np.sum(heuristic_scores)
        next_edge = filtered_edges[np.random.choice(len(filtered_edges), p=probabilities)]
        return next_edge

    def compute_heuristic_scores(self, edges):
        """
        Compute heuristic scores for each edge.
        """
        heuristic_scores = np.array([1 / self.edges[edge[1]].shape[0] for edge in edges])
