...
heuristic_walk = Heuristic_Walk(data.train_idx, data.inv_relation_id, transition_distr)
rl = Rule_Learner(heuristic_walk.edges, data.id2relation, data.inv_relation_id, dataset)
all_relations = sorted(heuristic_walk.edges)  # Learn for all relations
    ...
    for k in relations_idx:
        ...
        for length in rule_lengths:
            ...
            for _ in range(num_walks):
                walk_successful, walk = heuristic_walk.sample_walk(length + 1, rel)
                if walk_successful:
                    rl.create_rule(walk)
            it_end = time.time()
